...
library(tidyverse)
setwd("~/OneDrive - University of Toronto/MA/UofT Classes/Programming Class/ps9")
...
exer.avg <- ipip.comp %>% 
  ...
  summarise(
    n = length(unique(RID)),
    avg = mean(mean_trait_value),
    sem = sd(mean_trait_value)/sqrt(n-1)
  )
...
age.avg <- ipip.comp %>% 
  group_by(trait) %>%
  summarise(
    corrcoef = cor(age, mean_trait_value)
  )
